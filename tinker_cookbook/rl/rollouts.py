...
import logging
from tinker_cookbook.recipes.sql_rl.sql_env import SQLEnv
...
async def do_single_rollout(policy: TokenCompleter, env: Env) -> Trajectory:
    ...
    while True:
        try:
            ...
            logging.error(f"Error during policy completion: {e}")
            break
        ...
        if step_result.episode_done:
            break
    if isinstance(env, SQLEnv):
        rollout_trace = {env.question_id: env.traces}
    else:
        rollout_trace = None
    return Trajectory(transitions=transitions, final_ob=ob, trace=rollout_trace)
